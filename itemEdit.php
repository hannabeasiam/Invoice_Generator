<?php
  ...
  include("inc/functions.php");
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
      $invoice_detail_id = trim(filter_input(INPUT_POST,'invoice_detail_id'));
      $item = trim(filter_input(INPUT_POST,'item'));
      $invoice_description = trim(filter_input(INPUT_POST,'invoice_description'));
      $quantity = trim(filter_input(INPUT_POST,'quantity'));
      $price = trim(filter_input(INPUT_POST,'price'));
      $query = "UPDATE invoice_details SET item = '$item', invoice_description = '$invoice_description', quantity = '$quantity', price = '$price' WHERE invoice_detail_id='$invoice_detail_id'";
      if (empty($errorMessage)) { 
        try {
          $statement = $db->prepare($query);
          $statement->execute();
...
?>
	<!--main contents-->
	<div class="container">
  <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>" name="edit_item" method="post">
